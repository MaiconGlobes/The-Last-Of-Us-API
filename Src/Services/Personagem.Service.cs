...
namespace API_REST_The_Last_Of_Us.Src.Services
{
  public class PersonagemService
  {
    ...
    private PersonagemRepositories FPersonagemRepositories { get; set; }

    public PersonagemService()
    {
      Fcontexto = new Contexto();
      FPersonagemRepositories = new PersonagemRepositories();
    }
    ...
    public async Task<(byte Status, object Json)> BuscarTodosRegistrosAsync()
    {
      try
      {
        var listaDados = await FPersonagemRepositories.BuscarTodosRegistros();
